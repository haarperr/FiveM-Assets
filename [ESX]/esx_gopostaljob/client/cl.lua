...
local locations = {
	...
	["Sandy Shores"] = {
		...
		[12] = {2639.04, 4246.56, 44.77},
		[13] = {2455.85, 4058.3, 38.06},
		[14] = {3680.06, 4497.93, 25.11}, 
		[15] = {3807.8, 4478.6, 6.37},
		[16] = {2630.27,3262.88,56.25},
		[17] = {2622.43,3275.56,56.3},
		[18] = {2633.7,3287.4,56.45},		
	...
	["Paleto Bay"] = {
		['Max'] = 19,
		[1] = {1825.41, 3718.35, 34.42},
		[2] = {1899.13, 3764.68, 33.79},
		[3] = {1923.37, 3797.43, 33.44}, 
		[4] = {1914.69, 3813.37, 33.44},
		[5] = {1913.61, 3868.06, 33.37},
		[6] = {1942.34, 3885.42, 33.67},
		[7] = {1728.66, 3851.46, 34.78},
		[8] = {903.67, 3560.82, 33.81},
		[9] = {910.93, 3644.29, 32.68},
		[10] = {1393.15,3673.4, 34.79},
		[11] = {1435.18, 3682.92, 34.84},
		[12] = {2389.48, 3341.64, 47.72}, 
		[13] = {2393.01, 3320.62, 48.24},
		[14] = {2163.38, 3374.63, 46.07},
		[15] = {1959.95, 3741.99, 33.24},
		[16] = {1931.55, 3727.6, 33.84},
		[17] = {1850.68, 3690.03, 35.5}, 
		[18] = {1707.92, 3585.29, 36.57},
		[19] = {1756.33, 3659.54, 35.39},
...
function CreateBlip()
    if PlayerData.job ~= nil and PlayerData.job.name == 'gopostal' then
		...
		if BlipCloakRoom == nil then
			...
			AddTextComponentString('DHL Depot')
 ...
 while true do
  ...
  if PlayerData.job ~= nil and PlayerData.job.name == 'gopostal' then
   if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -402.732, 6172.939, 31.532, true) < 15) then
	DrawMarker(23, -402.732, 6172.939, 31.532-0.9, 0, 0, 0, 0.0, 0, 0, 3.0, 3.0, 0.5001, 255, 255, 255, 255, 0, 0, 0, 0)
	   ...
	   RemoveJobBlip()
		...
		exports['mythic_notify']:SendAlert('inform', 'Go around the back of the warehouse to fill your van', 7000)
      end
     end
	end
   end
   if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -441.282, 6142.714, 31.478, true) < 40) and IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("speedo")) and DoesEntityExist(goPostalVehicle) then
   ...
   if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), currentJob[1], currentJob[2], currentJob[3], true) < 20) and onJob then
   	DrawMarker(22, currentJob[1], currentJob[2], currentJob[3], 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 1555, 112, 100,210, true, true, 0,0)
	if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), currentJob[1], currentJob[2], currentJob[3], true) < 1.5) then
	 drawTxt('~w~Flex ~r~[E]~w~ to Deliver The Package')
	 if IsControlJustPressed(0, 38) then
   ...
   if onJob and not IsPedInAnyVehicle(GetPlayerPed(-1)) and GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), currentJob[1], currentJob[2], currentJob[3], true) < 20 then 
    ...
    if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), bootPos.x, bootPos.y, bootPos.z, true) < 2.5 and PackageObject == nil then 
     drawTxt('~w~Flex ~r~[E]~w~ To Take Package')
 ...
 while restockVan do 
  ...
  if carryingPackage.status then 
   ...
   if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), bootPos.x, bootPos.y, bootPos.z, true) < 2.6 then 
    drawTxt('~w~Flex ~r~[E]~w~ To Put Package Into Van')
    if IsControlJustPressed(0, 38) then 
     ...
     if restockPackages == 0 then 
      ...
      newShift()
      onJob = true
	  exports['mythic_notify']:SendAlert('inform', 'Van Restocked, Begin Delivering Your Packages', 5000)
     end
    end
   end
  end
...
function newShift()
 ...
 if currentJobPay > 2 then 
  currentJobPay = math.random(115, 126)
 end
end
...
function SpawnGoPostal(x,y,z,h)
 ...
 local plate2 = GetVehicleNumberPlateText(goPostalVehicle)
