...
ESX.RegisterServerCallback('esx_documents:deleteDocument', function(source, cb, id)
    ...
    MySQL.Async.execute('DELETE FROM user_documents WHERE id = @id AND owner = @owner',
    {
        ...
        if rowsChanged >= 1 then
            TriggerClientEvent('esx:showNotification:SendNotification', source, _U('document_deleted'))
            cb(true)
        else
            TriggerClientEvent('esx:showNotification:SendNotification', source, _U('document_delete_failed'))
            cb(false)
        end
        end)
end)
            ...
            if id ~= nil then
                MySQL.Async.fetchAll('SELECT * FROM user_documents where id = @id', {['@id']=id}, function (result)
                    ...
                    if(result[1] ~= nil) then
                        ...
                        TriggerClientEvent('esx:showNotification:SendNotification', _targetid, _U('copy_from_player'))
                        TriggerClientEvent('esx:showNotification:SendNotification', _source, _U('from_copied_player'))
                    else
                        TriggerClientEvent('esx:showNotification:SendNotification', _source, _U('could_not_copy_form_player'))
                    end
                end)
            else
                TriggerClientEvent('esx:showNotification:SendNotification', _source, _U('could_not_copy_form_player'))
