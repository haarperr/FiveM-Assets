...
AddEventHandler('esx_loffe_fangelse:hasEnteredMarker', function (zone)
  if zone == 'StartJob' then
    ...
    CurrentActionMsg  = ('Nacisnij ~INPUT_CONTEXT~ aby podniesc paczke.')
    CurrentActionData = {}
  elseif zone == 'LeaveBox' then
    CurrentAction	  = 'leave_box'
	CurrentActionMsg  = ('Nacisnij ~INPUT_CONTEXT~ aby odlozyc paczke.')
	CurrentActionData = {}
...
AddEventHandler('esx_loffe_fangelse:startJob', function()
	...
	repeat
	TriggerEvent("mt:missiontext", '~w~Do podniesienia zostalo: ~b~' .. TimeLeft .. ' ~w~sekund', 1000)
	TimeLeft = TimeLeft - 1
	Citizen.Wait(1000)
	until(TimeLeft == 0)
