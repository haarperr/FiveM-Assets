...
function refreshVaultInventory()
    data = exports['gr8_vault']:getGr8VaultLicense()
    ESX.TriggerServerCallback(
        "gr8_vault:getVaultInventory",
        function(inventory)
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            if data.item.type == "item_weapon" then
				...
				TriggerServerEvent("gr8_vault:putItem", --[[ESX.GetPlayerData().identifier,--]] vaultType, data.item.type, data.item.name, count)
            else
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            TriggerServerEvent("gr8_vault:getItem", --[[ESX.GetPlayerData().identifier,--]] vaultType, data.item.type, data.item.name, count)
        end
