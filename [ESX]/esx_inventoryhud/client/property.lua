...
RegisterNetEvent("gr8rp_inventoryhud:openPropertyInventory")
AddEventHandler("gr8rp_inventoryhud:openPropertyInventory", function(data, playerSafe, playerMotel, allHousing)
...
function refreshPropertyInventory()
    ...
    elseif isHousing then
        ...
        ESX.TriggerServerCallback("gr8rp_property:getPropertyInventory",function(inventory)
            setPropertyInventoryData(inventory)
    ...
    function(data, cb) 
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            elseif isHousing then
                ...
                TriggerServerEvent("gr8rp_property:putItem", ESX.GetPlayerData().identifier, data.item.type, data.item.name, count)
            end
        end
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            elseif isHousing then
                ...
                TriggerServerEvent("gr8rp_property:getItem", ESX.GetPlayerData().identifier, data.item.type, data.item.name, tonumber(data.number))
            end
        end
