...
RegisterCommand('steal', function(source, args)
	...
	if IsPedArmed(ped, 7) and not IsEntityDead(ped) and not IsPedInAnyVehicle(ped, true) then	 
		robo()
	else
	   exports['mythic_notify']:DoCustomHudText('error', 'You must be armed to rob someone', 5000)
	end
end)
...
function robo()
	...
	if closestPlayer ~= -1 and closestDistance <= 1.5 then
		...
		if distance <= 1.5 then
			if IsEntityPlayingAnim(searchPlayerPed, 'random@mugging3', 'handsup_standing_base', 3) or IsEntityPlayingAnim(searchPlayerPed, 'missminuteman_1ig_2', 'handsup_base', 3) or IsEntityDead(searchPlayerPed) or IsEntityPlayingAnim(searchPlayerPed, 'dead', 'dead_a', 3) or GetEntityHealth(searchPlayerPed) <= 0 then
			--if IsEntityPlayingAnim(searchPlayerPed, 'random@mugging3', 'handsup_standing_base', 3) or IsEntityDead(searchPlayerPed) or IsEntityPlayingAnim(searchPlayerPed, 'dead', 'dead_a', 3) or GetEntityHealth(searchPlayerPed) <= 0 then
				if IsEntityPlayingAnim(searchPlayerPed, 'dead', 'dead_a', 3) then
					TriggerServerEvent('robo:jugadorDead', target_id, playerheading, playerCoords, playerlocation)
					Citizen.Wait(4500)
				else
				...
				exports['mythic_notify']:DoCustomHudText('error', 'Hands must be up')	
			end
		end
	else
	exports['mythic_notify']:DoCustomHudText('error', 'Nobody Nearby')
	end
end

RegisterNetEvent('robo:doarrestedDead')
AddEventHandler('robo:doarrestedDead', function()
	local target, distance = ESX.Game.GetClosestPlayer()
	exports['progressBars']:startUI(1500, " Searching Pockets ")	
	Citizen.Wait(1000)
...
AddEventHandler('robo:doarrested', function()
	...
	exports['progressBars']:startUI(3500, " Searching Pockets ")	
	Citizen.Wait(3000)
	OpenBodySearchMenu(target)
end) 
...
function OpenBodySearchMenu(player)
	TriggerEvent("gr8rp_inventoryhud:openPlayerInventory", GetPlayerServerId(player), GetPlayerName(player))
end
