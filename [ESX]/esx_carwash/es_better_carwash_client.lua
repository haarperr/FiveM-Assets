	...
	while true do
		...
		if IsPedSittingInAnyVehicle(PlayerPedId()) then 
			for i = 1, #vehicleWashStation do
				...
				DrawMarker(1, garageCoords2[1], garageCoords2[2], garageCoords2[3], 0, 0, 0, 0, 0, 0, 5.0, 5.0, 2.0, 0, 255, 0, 0, 0, 0, 2, 0, 0, 0, 0)
				if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), garageCoords2[1], garageCoords2[2], garageCoords2[3], true ) < 5 then
					if mycie == false then
					HelpPromt('Presione ~INPUT_CONTEXT~ para ~b~limpiar ~s~el vehículo por ~g~(25$)')
					if IsControlJustPressed(1, Key) then
						TriggerServerEvent('es_better_carwash:checkmoney')
					end
				else
					HelpPromt('Presione ~INPUT_CONTEXT~ para ~o~parar')
					if IsControlJustPressed(1, Key) then
						mycie = false
						timer2 = false
    ...
    Citizen.CreateThread(function()
		while timer2 do
			...
			elseif (timer == 0) then
				...
				TriggerEvent("pNotify:SendNotification", {text = "¡Su vehículo ha sido lavado!", type = "warning", queue = "global", timeout = 5000, layout = "centerLeft"})
				StopParticleFxLooped(particles, 0)
				StopParticleFxLooped(particles2, 0)
				timer2 = false
			end
        ...
        while true do
			...
			if mycie == true then
				...
				if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), garageCoords3[1], garageCoords3[2], garageCoords3[3], true ) < 4 then
					DrawText3D(garageCoords3[1], garageCoords3[2], garageCoords3[3] + 3, '~b~Lavandose... ~s~Tiempo:~b~ '.. timer ..' ~s~segundos.')
				end
				end
			end
		end
...
AddEventHandler('es_better_carwash:notenoughmoney', function (moneyleft)
	TriggerEvent("pNotify:SendNotification", {text = "¡No tienes suficiente dinero! You need:" .. moneyleft .."$ mas.", type = "warning", queue = "global", timeout = 5000, layout = "centerLeft"}) 
end)
