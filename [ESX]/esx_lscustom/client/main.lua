...
Citizen.CreateThread(function()
	...
	if PlayerData.job.name == 'police' then isLeo = true end
end)
...
AddEventHandler('gr8rp:setJob', function(job)
    ...
    print('Tint knows my job is Leo')
  	end
end)
...
AddEventHandler('gr8rp:playerLoaded', function(xPlayer)
	...
	ESX.TriggerServerCallback('gr8rp_lscustom:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('gr8rp_lscustom:installMod')
AddEventHandler('gr8rp_lscustom:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('gr8rp_lscustom:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('gr8rp_lscustom:cancelInstallMod')
AddEventHandler('gr8rp_lscustom:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('gr8rp_lscustom:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent('gr8rp_lscustom:buyMod', price)
		...
		menu.close()
		TriggerEvent('gr8rp_lscustom:cancelInstallMod')
...
--[[RegisterCommand("tint", function()
	if isLeo then
		...
		local vehicle = GetVehiclePedIsIn(playerPed, false)
		props2['windowTint'] = 1
		ESX.Game.SetVehicleProperties(vehicle, props2)
	else
		print('not an LEO')
	end
end)]]
...
RegisterCommand('tint', function(source, args)
	if isLeo == true then
		...
		exports['mythic_notify']:SendAlert('error', 'Action Not Available', 5000)
	end
end)