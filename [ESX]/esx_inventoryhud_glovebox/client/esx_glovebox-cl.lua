...
AddEventHandler(
  ...
  function(xPlayer)
    PlayerData = xPlayer
    TriggerServerEvent("gr8rp_glovebox_inventory:getOwnedVehicle")
...
RegisterNetEvent("gr8rp_glovebox_inventory:setOwnedVehicle")
AddEventHandler(
  "gr8rp_glovebox_inventory:setOwnedVehicle",
  function(vehicle)
...
function OpenCoffresInventoryMenu(plate, max, myVeh)
  ESX.TriggerServerCallback(
    "gr8rp_glovebox:getInventoryV",
    function(inventory)
      text = _U("glovebox_info", plate, (inventory.weight / 100), (max / 100))
      data = {plate = plate, max = max, myVeh = myVeh, text = text}
      TriggerEvent("gr8rp_inventoryhud:openGloveboxInventory", data, inventory.blackMoney, inventory.cashMoney, inventory.items, inventory.weapons)
    end,
    plate
  )
end
