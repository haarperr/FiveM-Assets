...
Citizen.CreateThread(function()
  ...
  while true do
	...
	if (IsControlJustPressed(1, 48)) then
		...
		notowned = 0
		if #cars == 0 then
			ESX.ShowNotification("No hay vehículos para bloquear cerca.", "error")
		else
			for j=1, #cars, 1 do
				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							ESX.ShowNotification('Has bloqueado tu vehículo', "error")
						...
						elseif lock == 2 then
							...
							ESX.ShowNotification('Has desbloqueado tu vehículo', "ok")
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							SetVehicleLights(carstrie[i], 2)
