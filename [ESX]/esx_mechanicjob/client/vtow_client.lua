...
RegisterCommand("tow", function(source, args)
    ...
    if towing then
        ...
        if target ~= nil and towtruck ~= nil then
            local towPos = GetOffsetFromEntityInWorldCoords(towtruck, 0.0, -1.3, 2.5)
            SetEntityCoords(target, towPos, false, false, false, false)
            SetVehicleDoorsShut(target, true)
            --Citizen.Wait(100)
            local targetPos = GetEntityCoords(target, true)
            local attachPos = GetOffsetFromEntityGivenWorldCoords(towtruck, targetPos.x, targetPos.y, targetPos.z)
            SetVehicleDoorsShut(target, true)
            AttachEntityToEntity(target, towtruck, -1, attachPos.x, attachPos.y-1.0, attachPos.z+1.5, 0.0, 0.0, 0.0, false, false, false, false, 0, true)
            towSetupMode = false
            helpstate = 0
            towing = true
        elseif not towSetupMode then
            ShowNotification("Go to your ~r~towtruck ~s~and when you see the", "~y~yellow ~s~marker, press ~r~~h~ALT~h~", "~s~to select the vehicle.")
            towSetupMode = true
            towtruck = nil
            target = nil
            helpstate = 1
...
Citizen.CreateThread(function()
    while true do
        if towSetupMode then
            ...
            if helpstate ~= 0 then
                ...
                if hit and DoesEntityExist(veh) and IsEntityAVehicle(veh) then
                    ...
                    if (IsControlJustPressed(0, 19)) then
                        if helpstate == 1 then
                            ShowNotification("Go to your ~o~target vehicle~s~ and when you see the", "~y~yellow ~s~marker, press ~r~~h~ALT~h~", "~s~to confirm the vehicle to tow.")
                            towtruck = veh
                            helpstate = 2
                        elseif helpstate == 2 and veh ~= towtruck then
                            ShowNotification("Vehicles have been confirmed.", "~r~/tow ~s~to start towing, or press", "~r~~h~ALT~h~~s~ to cancel.")
                            target = veh
                            helpstate = 3
                        end
                    end
            ...
            elseif helpstate == 0 and IsControlJustPressed(0, 19) and towtruck ~= nil and target ~= nil then
                towtruck = nil
                target = nil
                helpstate = 1
            end
