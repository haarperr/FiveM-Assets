...
AddEventHandler(
  ...
  function(xPlayer)
    PlayerData = xPlayer
    TriggerServerEvent("gr8rp_trunk_inventory:getOwnedVehicule")
...
RegisterNetEvent("gr8rp_trunk_inventory:setOwnedVehicule")
AddEventHandler(
  "gr8rp_trunk_inventory:setOwnedVehicule",
  function(vehicle)
...
function OpenCoffreInventoryMenu(plate, max, myVeh)
  ESX.TriggerServerCallback(
    "gr8rp_trunk:getInventoryV",
    function(inventory)
      text = _U("trunk_info", plate, (inventory.weight / 100), (max / 100))
      data = {plate = plate, max = max, myVeh = myVeh, text = text}
      TriggerEvent("gr8rp_inventoryhud:openTrunkInventory", data, inventory.blackMoney, inventory.cashMoney, inventory.items, inventory.weapons)
    end,
    plate
  )
end
