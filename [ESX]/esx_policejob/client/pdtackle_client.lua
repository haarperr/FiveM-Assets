...
RegisterNetEvent('gr8rp_kekke_tackle:getTackled')
AddEventHandler('gr8rp_kekke_tackle:getTackled', function(target)
...
RegisterNetEvent('gr8rp_kekke_tackle:playTackle')
AddEventHandler('gr8rp_kekke_tackle:playTackle', function()
	local playerPed = GetPlayerPed(-1)
	...
	while true do
		...
		if IsControlPressed(0, Keys['LEFTSHIFT']) and IsControlPressed(0, Keys['G']) and not isTackling and GetGameTimer() - lastTackleTime > 10 * 1000 and PlayerData.job.name == PoliceJob then
			...
			if distance ~= -1 and distance <= 2.5 and not isTackling and not isGettingTackled and not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				...
				TriggerServerEvent('gr8rp_kekke_tackle:tryTackle', GetPlayerServerId(closestPlayer))
			end
		end
	end
end)