...
Citizen.CreateThread(function()
	...
	if PlayerData.job.name == 'tuner' then isTuner = true end
	if PlayerData.job.name == 'mechsandy' then isMechSandy = true end
  end)
...
AddEventHandler('gr8rp:playerLoaded', function(xPlayer)
	...
	ESX.TriggerServerCallback('gr8rp_lscustommech:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('gr8rp_lscustommech:installMod')
AddEventHandler('gr8rp_lscustommech:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('gr8rp_lscustommech:refreshOwnedVehicle', myCar)
	TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 20.0, 'airimpact', 0.1)
end)

RegisterNetEvent('gr8rp_lscustommech:cancelInstallMod')
AddEventHandler('gr8rp_lscustommech:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('gr8rp_lscustommech:installMod')
				else
					...
					if isRimMod then
						price = math.floor(vehiclePrice * data.current.price / 425)
						TriggerServerEvent('gr8rp_lscustommech:buyMod', price)
					elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
						price = math.floor(vehiclePrice * v.price[data.current.modNum + 1] / 425)
						TriggerServerEvent('gr8rp_lscustommech:buyMod', price)
					elseif v.modType == 17 then
						price = math.floor(vehiclePrice * v.price[1] / 425)
						TriggerServerEvent('gr8rp_lscustommech:buyMod', price)
					else
						price = math.floor(vehiclePrice * v.price / 425)
						TriggerServerEvent('gr8rp_lscustommech:buyMod', price)
					end
				end
		...
		menu.close()
		TriggerEvent('gr8rp_lscustommech:cancelInstallMod')
			...
			if v.modType then
				...
				elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
					...
					for j = 0, modCount, 1 do
						...
						if j == currentMods[k] then
							...
							price = math.floor(vehiclePrice * v.price[j+1] / 425)
				...
				elseif v.modType == 17 then -- TURBO
					...
					if currentMods[k] then
						...
						_label = 'Turbo - <span style="color:green;">$' .. math.floor(vehiclePrice * v.price[1] / 425) .. ' </span>'
					end
					table.insert(elements, {label = _label, modType = k, modNum = true})
				else
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if isMech or isTuner or isMechSandy then
